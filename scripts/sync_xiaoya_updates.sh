    ...
    if [ -n "$all_open" ]; then
        ...
        if [ -z "$cfg_open" ]; then
            ...
            if [ -n "$new_open" ]; then
                ...
                echo ">>> [token] CONST_REFRESH_TOKEN_OPEN 已更新"
                changed=1
            fi
        fi
    fi
    ...
    if [ -n "$all_token" ]; then
        ...
        if [ -z "$cfg_token" ]; then
            ...
            if [ -n "$new_token" ]; then
                ...
                echo ">>> [token] CONST_REFRESH_TOKEN 已更新"
                changed=1
            fi
        fi
    fi
...
INSERT INTO x_storages VALUES (
    ...
    json('{"open115_cookie":"$ESC_115_COOKIE","sync_interval":20,"root_folder_id":"$ESC_SYNC_ROOT_ID","qrcode_token":"","qrcode_source":"","page_size":1000,"limit_rate":0.5,"delete_after_sync":false,"run_once":true}'),
