...
func (d *Pan115) Put(ctx context.Context, dstDir model.Obj, stream model.FileStreamer, up driver.UpdateProgress) (model.Obj, error) {
	...
	fmt.Printf("size=%d, hash=%s", stream.GetSize(), fullHash)
	...
	if matched, err := fastInfo.Ok(); err != nil {
		...
		if u, ok := stream.(RapidUploader); ok {
			u.SetRapidUpload(true)
		}
	...
	if uploadResult == nil {
		return nil, fmt.Errorf("upload failed, response is nil")
	}
