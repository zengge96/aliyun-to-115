...
func (d *Pan115Share) List(ctx context.Context, dir model.Obj, args model.ListArgs) ([]model.Obj, error) {
	...
	fileResp, err := d.client.GetShareSnap(d.ShareCode, d.ReceiveCode, dir.GetID(), driver115.QueryLimit(int(d.Addition.PageSize)))
	if err != nil {
	...
	for total > count {
		...
		fileResp, err := d.client.GetShareSnap(
			...
			driver115.QueryLimit(int(d.Addition.PageSize)), driver115.QueryOffset(count),
		)
